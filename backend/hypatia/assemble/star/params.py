...
class SingleStarParams:
    ...
    def update_param(self, param_name: str, single_param: SingleParam, overwrite_existing=False):
    ...
    def calculated_params(self):
        ...
        if dist is not None and ra is not None and dec is not None:
                pos_values = spherical_astronomy_to_cartesian((ra, dec, dist))
                pos_single_param = SingleParam(value=pos_values, ref='Hypatia Calc')
                self.update_param("pos", pos_single_param)
                self.available_params.add("pos")
